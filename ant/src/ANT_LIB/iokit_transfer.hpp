...
class IOKitTransfer
{
   ...
   private:
      ...
      BOOL transfer_complete; //used to ensure a transfer abort does not discard properly transferred contents, only access with stMutexCriticalSection locked
      ...
      DSI_CONDITION_VAR stConditionCallbackComplete;

      //RESULT DATA
      IOReturn result_result;
      UInt32 result_io_size;
